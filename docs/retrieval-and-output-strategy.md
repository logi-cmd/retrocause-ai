...
- It has no runtime shared model-provider key dependency, so there is no central model-provider quota that all users hit together.
...
- No user-provided model/search keys in the active OSS browser/API surface.

### Future Pro Hosted Source Candidates
...
Hosted search candidates should be added only in the Pro line, behind source policies, provider budgets, encrypted credentials, queueing, and audit logs. They are not part of the keyless OSS browser/API surface.
...
Task 1 of the SourceBroker reliability pass adds a source profile registry in `retrocause.evidence_access`. Each active OSS source now has stable metadata for label, source kind, stability, cache policy, default RPM, monthly budget, and whether it requires managed credentials.
...
The broker still preserves explicit operator source overrides for built-in source names. The active OSS factory stays on built-in sources; hosted source routing belongs to the future Pro provider-budget layer.
...
- quota ownership labeling: central hosted quota, browser/account quota, BYOK quota in Pro, or local/uploaded evidence
...
### Historical Hosted Source Experiments

Hosted search adapter code has been removed from the active OSS source tree. The remaining lesson is architectural: Pro should own managed provider credentials, quota attribution, queueing, and retention rules before hosted search returns as a product capability.
...
- Source profiles centralize source labels, source kind, stability, cache policy, default RPM, monthly budget, and credential-boundary metadata.
...
- Hosted search remains outside the active OSS source factory. Pro can reintroduce hosted search through managed quota, BYOK, queueing, and audit controls.
