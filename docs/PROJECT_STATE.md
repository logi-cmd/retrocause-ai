...
1. sketch the first workspace/auth boundary for Pro API requests without enabling real auth yet
2. add durable event/status vocabulary for run history before connecting stores
3. prepare the first live-provider adapter candidate behind explicit auth, quota, and dry-run gates
