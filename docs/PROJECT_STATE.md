...
- A true live Chinese finance run with real provider/search keys still needs verification after the anchor-preservation fix; the homepage now has a guarded sample entry for this query path and the OpenRouter picker prefers `deepseek/deepseek-chat` over the observed DeepSeek V3 0324 empty-result path, but live source quality is still unverified without a successful real-key run.
