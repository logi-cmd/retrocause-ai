...
1. plan the hosted store migration path from local JSON/in-memory queue state to Postgres plus Redis, tenant/auth boundaries, and worker ownership
2. design the first hosted source/provider adapter contract around tenant quota ownership, cooldowns, and partial-result reporting
3. add a non-executing adapter dry-run shape before any live provider calls
