...
Current UX focus: Pro should keep the knowledge graph as the primary workspace. The first Rust web shell is server-rendered and graph-first; it can now create runs through the Rust API, reload run detail/graph payloads, update the graph workspace from API state, show keyless provider/search quota ownership plus cooldown semantics, and let operators select graph nodes into a browser-local inspector. Future work should add persistence, real provider/search routing, and deeper graph review without inheriting the OSS page layout.
...
1. replace process-local run storage with the first durable Pro run-store boundary
2. design the provider/search routing executor behind the existing quota ownership and cooldown payloads before adding any real credentials
3. deepen graph review interactions around evidence/challenge focus without inheriting the OSS evidence-board layout
