...
Current UX focus: Pro should keep the knowledge graph as the primary workspace. The first Rust web shell is server-rendered and graph-first; it can now create runs through the Rust API, reload run detail/graph payloads, update the graph workspace from API state, show keyless provider/search quota ownership plus cooldown semantics, let operators select graph nodes into a browser-local inspector, focus evidence/challenge items from inspector links, keep created runs across API restarts through a local JSON run-store boundary, preview provider/source routing plans without executing providers, create local preview-only execution jobs behind that routing plan, expose non-executing worker work orders for those jobs, render queue jobs plus route-step/work-order details, show a planned hosted-worker lifecycle/failure-state contract, show a planned hosted storage/tenant/worker-ownership migration contract, show a planned provider-adapter quota/degradation/partial-result contract, run a non-executing provider-adapter dry-run from the graph workspace, show a non-enforcing workspace/auth preview context, and render a non-durable run event timeline/status vocabulary derived from the current run record. Future work should add real provider/search execution and deeper graph review without inheriting the OSS page layout.
...
1. prepare the first live-provider adapter candidate behind explicit auth, quota, dry-run, and status-event gates
2. design the next graph review workflow for comparing evidence and challenges across runs
