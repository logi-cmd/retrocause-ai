...
1. turn the worker-result commit-intent gate into durable worker-owned result-event commits only after real auth, quota, vault, worker leases, idempotent storage writes, and hosted result snapshots exist
